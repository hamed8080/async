...
final class NativeWebSocketProvider: NSObject, WebSocketProvider, URLSessionDelegate, URLSessionWebSocketDelegate {
    ...
    /// -1001(NSURLErrorDomain) request timeout: It may occur when the client can't see the server whether there is no internet connectivity or server is down.
    func handleError(_ error: Error?) {
        if let error = error as NSError? {
            if [53, 54, 57, 60, 89, -1001].contains(error.code) {
                isConnected = false
                closeConnection()
